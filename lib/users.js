    ...
    create(user){
        return new Promise((resolve, rejects) => {
            const callback = (error) => {
                ...
                this.instance.http.post(`/api/user/add`,JSON.stringify(user), (error, res, body) => {
                    if (error) {
                        console.log(error);
                        rejects(error);
                    }
    ...
    checkPassword(login, password, type = "main") {
        return new Promise((resolve, rejects) => {
            const callback = (error) => {
                ...
                this.instance.http.get(encodeURI(`/api/user/check_password?login=${login}&type=${type}&password=${password}`), (error, res, body) => {
                    if (error) {
                        console.log(error);
                        rejects(error);
                    }
            ...
            if (value === true) {
                return new Promise((resolve, reject) => {
                    const callback = (error) => {
                        ...
                        this.instance.http.get(encodeURI(`/api/user/change_password?login=${login}&type=${type}&password=${newPassword}`), (error, res, body) => {
                            if (error) {
                                console.log(error);
                                rejects(error);
                            }
